			...
			<table class="table table-bordered table-stripped">
				...
				<tbody>
					...
					$i = 1;
						...
						$query = "SELECT SUM(quantity)as value_sum FROM order_list WHERE product_id = {$row['product_id']}";
						$select_order = mysqli_query($conn, $query);
						if($select_order->num_rows > 0){
							$row1 = mysqli_fetch_assoc($select_order);
							$ordered = $row1['value_sum'];
						} else{
							$ordered = 0;
						}
						echo $row['quantity'];
						echo $ordered;
						$avail = $row['quantity'] - $ordered;
						//$avail = 10;
